...
let handler = async (m, { conn, usedPrefix: _p, __dirname, args }) => {
 ...
 if (teks == 'nsfw') tags = {
  ...
  try {
      ...
      // OPTS UCAPAN
      let waktujam = moment.tz('Asia/Jakarta').format('HH')
      let waktu = `${waktujam}`
      let _ucapan = (waktu == 4 ? '🌄 Good Morning' : '' || waktu == 5 ? '🌄 Good Morning' : '' || waktu == 6 ? '🌄 Good Morning' : '' || waktu == 5 ? '🌄 Good Morning' : '' || waktu == 7 ? '🌄 Good Morning' : '' || waktu == 8 ? '🌄 Good Morning' : '' || waktu == 9 ? '🌄 Good Morning' : '' || waktu == 10 ? '🌄 Good Morning' : '' || waktu == 11 ? '☀️ Good Afternoon' : '' || waktu == 12 ? '☀️ Good Afternoon' : '' || waktu == 13 ? '☀️ Good Afternoon' : '' || waktu == 14 ? '☀️ Good Afternoon' : '' || waktu == 15 ? '🌇 Good Afternoon' : '' || waktu == 16 ? '🌇 Good Afternoon' : '' || waktu == 17 ? '🌇 Good Afternoon' : '' || waktu == 18 ? '🌙 Good Night' : '' || waktu == 19 ? '🌙 Good Night' : '' || waktu == 20 ? '🌙 Good Night' : '' || waktu == 21 ? '🌙 Good Night' : '' || waktu == 22 ? '🌙 Good Night' : '' || waktu == 23 ? '🌌 Midnight' : '' || waktu == 24 ? '🌌 Midnight' : '' || waktu == 0 ? '🌌 Midnight' : '' || waktu == 1 ? '🌌 Midnight' : '' || waktu == 2 ? '🌌 Midnight' : '' || waktu == 3 ? '🌌 Midnight' : '' )
      let ucapan = `${_ucapan}`
...
const listMessage = {
  text: `*${ucapan} ${conn.getName(m.sender)} !*\nSilahkan Pilih Menu Dibawah (≧ω≦)ゞ`,
  footer: botdate,
  title: `${htki} *LIST MENU* ${htka}`,
  buttonText: `${htjava} MENU`,
  sections
}
  if (teks == '404') {
 ...
 let mode = global.opts['self'] ? 'Private' : 'Publik'
    ...
    let replace = {
      ...
      ucapan,platform, wib, mode, _p, money, age, tag, name, prems, level, limit, name, weton, week, date, dateIslamic, time, totalreg, rtotalreg, role,
      readmore: readMore
    }
