...
try {
    if (/cekapi(key)?|cekkey/i.test(command)) {
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
       case 'xteam':
         let xteam = await (await fetch(`https://api.xteam.xyz/cekey?APIKEY=${args[0]}`)).json().catch(_ => m.reply(`Maaf restapi ini sedang error, harap coba lagi nanti`))
         await m.reply(cek)
         if (!xteam.response) throw 'ERROR!'
...
• *ᴇxᴘɪʀᴇᴅ:* ${(xteam.response.expired).replace('Premium left:', '')}`, m)
    ...
    m.reply(cek)
    if (!lol.result) throw `Maaf restapi ini sedang error, harap coba lagi nanti`
    if (lol.result == "error") throw `ɪɴᴠᴀʟɪᴅ ᴀᴘɪᴋᴇʏ !`
